...
import { BarChart, DollarSign, TrendingDown, TrendingUp, ListPlus, Landmark, Wallet, Percent, PiggyBank, Target as TargetIcon, Activity, CreditCard, ShieldCheck, Loader2 } from 'lucide-react';
...
interface InteractiveLegendProps {
  data: Array<{ name: Category; value: number; color: string }>; 
...
export default function DashboardPage() {
  const { 
    transactions, accounts, budgets, creditCards, 
    systemMonth, systemYear, getAccountById, settleCreditCardDues 
  } = useAppData();
  ...
  const totalDebt = useMemo(() => {
    return creditCards.reduce((sum, card) => sum + card.spent, 0);
  }, [creditCards]);

  const debtToIncomeRatio = useMemo(() => {
    if (totalIncome === 0) return -1; // Special case for no income
  ...
  const handleSettleDues = async (cardId: string, cardName: string, amount: number) => {
    setIsSettling(cardId);
    try {
        await settleCreditCardDues(cardId, cardName, amount);
        toast({
            title: "Dues Settled",
            description: `A payment of ₹${amount.toFixed(2)} for your ${cardName} card has been recorded.`
        });
    } catch (error) {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4">
       ...
       {creditCards.length > 0 && (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Your credit card spending and available limits for {currentMonthName}.</CardDescription>
            </CardHeader>
                                ...
                                <AlertDialog>
                                    <AlertDialogTrigger asChild>
                                        <Button 
                                            variant="outline" 
                                            size="sm"
                                            disabled={card.spent <= 0 || isSettling !== null}
                                        >
                                            {isSettling === card.id 
                                    ...
                                    <AlertDialogContent>
                                        <AlertDialogHeader>
                                            <AlertDialogTitle>Settle Dues for {card.name}?</AlertDialogTitle>
                                            <AlertDialogDescription>
                                                This will create an expense of <strong>₹{card.spent.toFixed(2)}</strong> from your <strong>{primaryAccount?.name}</strong> account to pay the credit card bill. This action cannot be undone.
                                            </AlertDialogDescription>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
                                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                                            <AlertDialogAction onClick={() => handleSettleDues(card.id, card.name, card.spent)}>
                                                Confirm Payment
                                            </AlertDialogAction>
                                        </AlertDialogFooter>
                                    </AlertDialogContent>
       ...
       <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="border rounded-lg p-4 flex flex-col justify-between">
                <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Debt-to-Income (DTI)</CardTitle>
                    <TrendingDown className="h-4 w-4 text-muted-foreground" />
                </div>
                <div>
                    <div className={cn("text-2xl font-bold", debtToIncomeRatio > 43 ? 'text-destructive' : 'text-[hsl(var(--chart-3))]')}>
                        {debtToIncomeRatio < 0 ? 'N/A' : `${debtToIncomeRatio.toFixed(1)}%`}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {debtToIncomeRatio < 0 ? 'Income required for calculation' : `Based on monthly credit spending`}
                    </p>
                </div>
            </div>
